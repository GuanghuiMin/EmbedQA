...
class BiDAF(nn.Module):
    def __init__(self, args):
        ...
        self.modeling_layer = nn.GRU(8*self.d, self.d, num_layers=2, bidirectional=True, dropout=0.3, batch_first=True)

        self.p1_layer = nn.Linear(10*self.d, 1, bias=False)
        self.p2_lstm_layer = nn.GRU(2*self.d, self.d, bidirectional=True, dropout=0.3, batch_first=True)
    ...
    def build_contextual_embd(self, x_c, x_w):
        ...
        embd = torch.cat((char_embd, word_embd), 2) # (N, seq_len, d=embd_size)
        embd = self.highway_net(embd) # (N, seq_len, d=embd_size)
